...
**Fases 2 (Backend core), 3 (Busca e experiência pública), 4 (API de integração com CRMs parceiros), 5 (CRM interno), 6 (hardening de produção), 6.1 (SEO avançado) e 6.2 (convite de acesso por e-mail, "esqueci senha" e aviso de login) implementadas e testadas.** A API real está no ar (`backend/`) com banco de dados, CRUD de imóveis, leads e autenticação — e o site público + painel do corretor (`frontend/site/`) já consomem essa API de verdade, não mais dados fictícios estáticos. A busca tem filtros avançados (bairro, área, ordenação), há favoritos salvos no navegador, mapa na página do imóvel (OpenStreetMap) e SEO avançado: título/description otimizados para os termos que as pessoas de fato buscam, Open Graph e Twitter Card, dados estruturados (JSON-LD) gerados **no servidor** para imóveis e buscas, `sitemap.xml` com prioridade por página e `robots.txt` (ver `docs/ROADMAP.md`, seção Fase 6.1, para a lista completa e a limitação conhecida sobre `og:image`). CRMs parceiros já podem importar/exportar imóveis e receber webhooks de leads e mudanças de status via chave de API própria (aba "Parceiros (CRMs)" no painel, documentação interativa em `/docs.html`). O painel também tem um funil de atendimento em quadro Kanban (Novo → Em atendimento → Convertido/Perdido), atribuição de leads a um corretor, histórico de atendimento por lead, e uma aba "Equipe" para o administrador cadastrar e gerenciar quem tem acesso ao painel — cadastrando sem definir senha, a pessoa recebe um convite por e-mail para escolher a própria senha. Login também tem "esqueci minha senha" e um aviso por e-mail a cada acesso (ver `docs/ROADMAP.md`, seção Fase 6.2, sobre a senha de aplicativo do Gmail necessária para o envio real). Para publicar de verdade, o projeto já tem variáveis de ambiente, rate limiting, cabeçalhos de segurança, backup de banco e artefatos de deploy (Docker/systemd + guia passo a passo em `docs/DEPLOY.md`) prontos — falta só você decidir domínio e hospedagem. Para rodar tudo localmente:
