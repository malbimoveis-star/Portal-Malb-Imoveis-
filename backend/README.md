...
│   ├── seo.js             # título/description/OG/Twitter/JSON-LD gerados no servidor — Fase 6.1
...
**Fase 6.1 — SEO avançado (sessão seguinte, a pedido):** o pedido original era adicionar uma meta tag de palavras-chave; como essa tag não tem efeito no Google desde 2009, o trabalho foi redirecionado para o que de fato ajuda (ver `docs/ROADMAP.md` para a lista completa). O HTML servido por `GET /imovel.html` e `GET /busca.html` foi conferido via `curl` (sem executar JavaScript, como um robô de prévia de link faria) em 6 cenários: imóvel real, busca sem filtro nenhum, busca com filtro "forte" (bairro+tipo+finalidade, que entra no `<link rel="canonical">`) e com filtro "fraco" (preço/quartos, que corretamente fica de fora do canonical), e `id` inexistente ou ausente (fallback para os valores padrão da página). Todo JSON-LD gerado (`RealEstateListing` do imóvel, `BreadcrumbList` da busca, `RealEstateAgent` e `FAQPage` da home) validado como JSON bem-formado. Suíte via Playwright confirmando que nada quebrou funcionalmente (cards, título da aba, texto alternativo dinâmico das fotos) e que o painel e a API continuam funcionando normalmente. Limitação conhecida (documentada em `docs/ROADMAP.md`): não há `og:image` funcional porque as fotos são guardadas como base64 no banco, não como arquivos com URL própria.
